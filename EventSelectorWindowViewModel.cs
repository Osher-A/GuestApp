...
namespace GuestApp.ViewModel
{
    public class EventSelectorWindowViewModel : INotifyPropertyChanged
    {
        ...
        {
            UpdateCurrentAppEvent();
            View.App.Current.MainWindow.Hide();

            //When one comes back to the event selector it should be visible.
            CreateAndEditEventWindowVisibility = Visibility.Collapsed;
            
            View.GuestListsWindow listsWindow = new View.GuestListsWindow();
            var vm = new GuestListWindowViewModel(new GuestRepository(), new UsersMessageService());
            listsWindow.DataContext = vm;
